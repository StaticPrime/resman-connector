...
All ResMan types and enums are available through the `resmanTypes` namespace for convenient access:

```typescript
import { resmanTypes } from 'resman-connector';

// Use types from the namespace
type Balance = resmanTypes.TBalanceResponse;
type WorkOrder = resmanTypes.TWorkOrderResponse;
type Lease = resmanTypes.TLeaseResponse;

// Use request types
type WorkOrderCreate = resmanTypes.TWorkOrderCreateRequest;
type VendorCreate = resmanTypes.TVendorRequest;

// Use enums
const status = resmanTypes.WorkOrderStatus.Open;
const priority = resmanTypes.WorkOrderPriority.High;
const leaseStatus = resmanTypes.LeaseStatus.Current;

// Example function using resmanTypes
function processBalance(balance: resmanTypes.TBalanceResponse): number {
  return balance.Balance;
}
```

You can also import types and enums individually if preferred:
